...
def check(message,inputted_message,img):
    if message == inputted_message:
        cv2.imwrite(str(message)+'.png',img)
...
def write(x, img):
    ...
    check(label,inputted_message,img)
    return img
...
if __name__ == '__main__':
    inputted_message = reciveer()
    host = '192.168.1.213'; 
    #host= '172.17.120.56'
    port = 6000
    print(' Host ip is ..'+ host + '  on port  '+str(port))
    ...
    cap = cv2.VideoCapture('http://'+host+':8080/video')
    ...
    start = time.time()    
    while cap.isOpened():
        #time.sleep(0.19078696999999986)
